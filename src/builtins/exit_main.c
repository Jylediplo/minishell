...
/*   Updated: 2024/05/31 11:05:22 by pantoine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	free_all_exit(t_lexer **lexer, t_list *cmdlist,
			t_shell *shell, int is_child)
{
	free(shell->children);
	free_lexer(lexer);
		...
		t_lexer **lexer, t_shell *shell)
{
	...
	if (len == 1)
		exit_no_arg(shell, lexer, cmdlist, cmd);
	if (cmd->command[1][0] == '-')
		i++;
	...
	exit_single_arg(shell, lexer, cmdlist, cmd);
	return (0);
}
